...
// https://github.com/dotnet/runtime/blob/2aade6beb02ea367fd97c4070a4198802fe61c03/src/libraries/Microsoft.Extensions.Configuration.Binder/src/ConfigurationBinder.cs
//
...
public static class ConfigurationBinder
{
    ...
    /// <param name="provider">The service provider to use for instantiating objects.</param>
    /// <returns>The new instance of T if successful, default(T) otherwise.</returns>
    [RequiresDynamicCode(DynamicCodeWarningMessage)]
    [RequiresUnreferencedCode(TrimmingWarningMessage)]
    public static T? Get<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(this IConfiguration configuration, IServiceProvider provider)
        => configuration.Get<T>(provider, null);
    ...
    [RequiresDynamicCode(DynamicCodeWarningMessage)]
    [RequiresUnreferencedCode(TrimmingWarningMessage)]
    public static object? Get(this IConfiguration configuration, IServiceProvider provider, Type type)
        => configuration.Get(provider, type, null);
        ...
        Action<BinderOptions>? configureOptions)
    {
        ...
        BindInstance(provider, type, bindingPoint, config: configuration, options: options, isParentCollection: false);
    ...
    [RequiresDynamicCode(DynamicCodeWarningMessage)]
    [RequiresUnreferencedCode(InstanceGetTypeTrimmingWarningMessage)]
    public static void Bind(this IConfiguration configuration, IServiceProvider provider, object? instance)
        => configuration.Bind(provider, instance, null);
    ...
    public static void Bind(this IConfiguration configuration, IServiceProvider provider, object? instance, Action<BinderOptions>? configureOptions)
    {
        ...
        if (instance != null)
        {
            ...
            BindInstance(provider, instance.GetType(), bindingPoint, configuration, options, false);
        }
    }
    ...
    [RequiresUnreferencedCode(TrimmingWarningMessage)]
    public static T? GetValue<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(this IConfiguration configuration, IServiceProvider provider, string key)
    {
        return GetValue(configuration, provider, key, default(T));
    }
    ...
    /// <returns>The converted value.</returns>
    [RequiresUnreferencedCode(TrimmingWarningMessage)]
    public static T? GetValue<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(this IConfiguration configuration, IServiceProvider provider, string key, T defaultValue)
    {
        return (T?)GetValue(configuration, provider, typeof(T), key, defaultValue);
    }
    ...
    public static object? GetValue(
        ...
        string key)
    {
        return GetValue(configuration, provider, type, key, defaultValue: null);
    }
        ...
        object? defaultValue)
    {
        ...
        if (value != null)
        {
            return ConvertValue(provider, type, value, section.Path);
        }
        return defaultValue;
    }
    ...
    private static void BindProperties(IServiceProvider provider, object instance, IConfiguration configuration, BinderOptions options)
    {
        ...
        if (options.ErrorOnUnknownConfiguration)
        {
            ...
            {
                throw new InvalidOperationException($"Missing configuration {nameof(options.ErrorOnUnknownConfiguration)}, {nameof(BinderOptions)}, {instance.GetType()}, {string.Join(", ", missingPropertyNames)}");
            }
        }

        foreach (PropertyInfo property in modelProperties)
        {
            BindProperty(provider, property, instance, configuration, options);
        }
    }

    [RequiresDynamicCode(DynamicCodeWarningMessage)]
    [RequiresUnreferencedCode(PropertyTrimmingWarningMessage)]
    private static void BindProperty(IServiceProvider provider, PropertyInfo property, object instance, IConfiguration config, BinderOptions options)
    {
        ...
        bool isParentCollection)
    {
        ...
        if (configValue != null && TryConvertValue(provider, type, configValue, section?.Path, out object? convertedValue, out Exception? error))
        {
            if (error != null)
            {
                throw error;
        ...
        if (config.GetChildren().Any())
        {
            ...
            if (type.IsArray || IsImmutableArrayCompatibleInterface(type))
            {
                ...
                {
                    bindingPoint.SetValue(BindArray(provider, type, (IEnumerable?)bindingPoint.Value, config, options));
                }
            ...
            if (TypeIsASetInterface(type))
            {
                ...
                {
                    object? newValue = BindSet(provider, type, (IEnumerable?)bindingPoint.Value, config, options);
            ...
            if (TypeIsADictionaryInterface(type))
            {
                if (!bindingPoint.IsReadOnly || bindingPoint.Value is not null)
                {
                    object? newValue = BindDictionaryInterface(provider, bindingPoint.Value, type, config, options);
                    if (!bindingPoint.IsReadOnly && newValue != null)
                    {
                        bindingPoint.SetValue(newValue);
                    }
            ...
            if (bindingPoint.Value is null)
            {
                ...
                {
                    bindingPoint.SetValue(CreateInstance(provider, type, config, options));
                }
            }
            ...
            {
                BindDictionary(provider, bindingPoint.Value, dictionaryInterface, config, options);
            }
            else
            {
                ...
                {
                    BindCollection(provider, bindingPoint.Value, collectionInterface, config, options);
                }
                else
                {
                    BindProperties(provider, bindingPoint.Value, config, options);
                }
            }
        }
        else
        {
            ...
            {
                bindingPoint.TrySetValue(CreateInstance(provider, type, config, options));
            }
        }
    }
        ...
        BinderOptions options)
    {
        ...
        {
            throw new InvalidOperationException($"Cannot activate abstract or interace {type}");
        }
        ...
        {
            throw new InvalidOperationException($"Missing public instance constructor for type {type}");
        }

        if (constructors.Length > 1 && !hasParameterlessConstructor)
        {
            throw new InvalidOperationException($"Multiple parameterized constructor for type {type}");
        }
        ...
        if (constructors.Length == 1 && !hasParameterlessConstructor)
        {
            ...
            {
                throw new InvalidOperationException($"Cannot bind to constructor parameter for type {type} and parmeter {nameOfInvalidParameter}");
            }
            ...
            {
                throw new InvalidOperationException("Constructor parameters do not match properties for type {type} and parameter {nameOfInvalidParameters}");
            }
            ...
            {
                parameterValues[index] = BindParameter(provider, parameters[index], type, config, options);
            }
        ...
        {
            throw new InvalidOperationException($"Failed to activate type {type}", ex);
        }

        return instance ?? throw new InvalidOperationException($"Failed to activate type {type}");
    }
        ...
        IConfiguration config, BinderOptions options)
    {
        ...
        BindDictionary(provider, source, dictionaryType, config, options);
...
#pragma warning restore CA1305 // Specify IFormatProvider
            ...
            catch (Exception ex)
            {
                if (options.ErrorOnUnknownConfiguration)
                {
                    throw new InvalidOperationException($"General error when binding {nameof(options.ErrorOnUnknownConfiguration)}", ex);
                }
            }
        }
    }
        ...
        IConfiguration config, BinderOptions options)
    {
        ...
        foreach (IConfigurationSection section in config.GetChildren())
        {
            ...
            catch (Exception ex)
            {
                if (options.ErrorOnUnknownConfiguration)
                {
                    throw new InvalidOperationException($"General error while binding {nameof(options.ErrorOnUnknownConfiguration)}", ex);
                }
    ...
    private static Array BindArray(IServiceProvider provider, Type type, IEnumerable? source, IConfiguration config, BinderOptions options)
    {
        Type elementType;
        if (type.IsArray)
        {
            ...
            {
                throw new InvalidOperationException($"Unsupported multidimentional array, {type}");
            }
    ...
    [RequiresDynamicCode(DynamicCodeWarningMessage)]
    [RequiresUnreferencedCode("Cannot statically analyze what the element type is of the Array so its members may be trimmed.")]
    private static object? BindSet(IServiceProvider provider, Type type, IEnumerable? source, IConfiguration config, BinderOptions options)
    {
    ...
    private static bool TryConvertValue(
        [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)]
        IServiceProvider provider, Type type,
        string value, string? path, out object? result, out Exception? error)
    {
        ...
        if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(Nullable<>))
        {
            ...
            return TryConvertValue(provider, Nullable.GetUnderlyingType(type)!, value, path, out result, out error);
        }
        ...
        if (converter.CanConvertFrom(typeof(string)))
        {
            ...
            {
                error = new InvalidOperationException($"Failed binding {path} and {type}", ex);
            }
            return true;
        }
        ...
        // Following code is added from the original implementation of ConfigurationBinder
        // We needed the configuration binder to be able instance objects using the underlying IServiceProvider
        //
        // Also we always return true so we do not create errors incase a converter is not found
        ...
        try
        {
            result = ActivatorUtilities.CreateInstance(provider, type, value);
        }
        catch (Exception e)
        {
            error = new InvalidOperationException($"Failed to convert from string to type {type.FullName}, please check you have one string in constructor!", e);
        }
    ...
    {
        TryConvertValue(provider, type, value, path, out object? result, out Exception? error);
        if (error != null)
        {
            throw error;
        }
    ...
    [RequiresDynamicCode(DynamicCodeWarningMessage)]
    [RequiresUnreferencedCode(PropertyTrimmingWarningMessage)]
    private static object? BindParameter(IServiceProvider provider, ParameterInfo parameter, Type type, IConfiguration config,
        BinderOptions options)
    {
        ...
        {
            throw new InvalidOperationException($"Parameter being bound to unnamed {type}");
        }
        ...
        if (propertyBindingPoint.Value is null)
        {
            ...
            {
                throw new InvalidOperationException($"Parameter has no matching config {type}, {parameterName}");
            }
        }
