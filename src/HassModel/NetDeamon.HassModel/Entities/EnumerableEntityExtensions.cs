...
public static class EnumerableEntityExtensions
{
    ...
    /// <param name="service">Name of the service to call. If the Domain of the service is the same as the domain of the Entities it can be omitted</param>
    /// <param name="data">Data to provide</param>
    public static async Task<IEnumerable<JsonElement?>> CallServiceWithResponseAsync(this IEnumerable<IEntityCore> entities, string service, object? data = null)
    {
        ArgumentNullException.ThrowIfNull(service);
        ...
        // service for each group separately
        var serviceCalls = entities.GroupBy(e => e.HaContext);
        var returnValues = new List<JsonElement?>();

        foreach (var group in serviceCalls)
        {
            returnValues.Add(await group.Key.CallServiceWithResponseAsync(serviceDomain, serviceName, new ServiceTarget { EntityIds = group.Select(e => e.EntityId).ToList() }, data));
        }
        return returnValues;
