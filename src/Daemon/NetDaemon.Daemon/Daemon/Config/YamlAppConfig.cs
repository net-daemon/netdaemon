        ...
        public void SetPropertyConfig(YamlMappingNode appNode, string? appId, ApplicationContext applicationContext)
        {
            foreach (KeyValuePair<YamlNode, YamlNode> entry in appNode.Children)
            {
                ...
                try
                {
                    ...
                    var prop = applicationContext.ApplicationInstance.GetType().GetYamlProperty(scalarPropertyName) ??
                               throw new MissingMemberException(
                                   $"{scalarPropertyName} is missing from the type {applicationContext.ApplicationInstance.GetType()}");

                    var instance = InstanceProperty(applicationContext.ApplicationInstance, prop.PropertyType, entry.Value, applicationContext);

                    prop.SetValue(applicationContext.ApplicationInstance, instance);
                }
                catch (Exception e)
                {
